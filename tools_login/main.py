...
class AutoClickerGUI(QMainWindow):
    def execute_text_action(self, action):
        if not self.is_running:
            return
        import pyautogui
        pyautogui.write(action.comment, interval=0.05)
    def update_actions_table(self):
        ...
        for i, action in enumerate(self.actions_manager.actions):
            ...
            self.action_table.setItem(i, 2, QTableWidgetItem(str(action.x)))
            self.action_table.setItem(i, 3, QTableWidgetItem(str(action.y)))
            self.action_table.setItem(i, 4, QTableWidgetItem(str(action.click_type)))
            self.action_table.setItem(i, 5, QTableWidgetItem(str(action.repeat)))
            self.action_table.setItem(i, 6, QTableWidgetItem(str(action.delay)))
            self.action_table.setItem(i, 7, QTableWidgetItem("Có" if action.move_back else "Không"))
            # Thêm nút điều khiển/xoá
    ...
    layout = QVBoxLayout()
        ...
        # Nhóm nhập text tự động
        text_group = QGroupBox("Thêm hành động bàn phím")
        text_layout = QHBoxLayout()
        text_layout.addWidget(QLabel("Text tự động:"))
        self.text_input = QLineEdit()
        self.text_input.setPlaceholderText("Nhập nội dung cần tự động nhập...")
        text_layout.addWidget(self.text_input)
        self.add_text_btn = QPushButton("Thêm hành động bàn phím")
        self.add_text_btn.setMinimumHeight(40)
        self.add_text_btn.setMinimumWidth(180)
        self.add_text_btn.setStyleSheet("background-color: #FF9800; color: white; font-size: 18px; font-weight: bold;")
